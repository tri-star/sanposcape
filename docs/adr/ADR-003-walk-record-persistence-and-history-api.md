...
- **概要**: `walks` の行を DELETE で物理的に削除する。`deleted_at` のような論理削除カラムは持たない。
- **メリット**: 既存クエリ（一覧・集計・streak）に除外条件を足す必要がなく、除外漏れによる不整合が構造的に起きない。アカウント削除（CASCADE の物理削除）と挙動が対称。
- **デメリット**: 復元不可。監査ログが残らない。

#### 選択肢2: 論理削除（`deleted_at`）

- **概要**: `deleted_at` カラムを追加し、削除は UPDATE で済ませる。読み取り側は `deleted_at IS NULL` を条件に加える。
- **メリット**: 誤削除から復元できる。将来のゴミ箱 UI に繋がる。
- **デメリット**: `get_by_id` / `list_for_user` / `aggregate_daily_for_user` / `list_walk_dates_desc` / `get_by_client_walk_id` の全クエリに除外条件が必要で、1箇所の漏れが集計値を狂わせる（決定11で非正規化を避けたのと同種のリスク）。MVP に復元要件が無い。
...
**（SS-53 追補）削除を物理削除にしたのは、除外条件の書き漏れリスクを新たに作らないため。** 論理削除は読み取り側の全クエリに条件を配る形になり、`walks` は既に一覧・詳細・日次集計・streak 走査・冪等キー照合（`get_by_client_walk_id`）と読み取り経路が5本ある。1本でも漏らすと「消したはずの散歩が集計に残る」という発見しにくい不整合になる。復元要件が無い現時点では、除外条件をゼロに保てる物理削除の方が安全と判断した。

...
アプリ強制終了からの復帰は mobile 側のローカル永続化（SS-19）で扱う。
...
`distance_meters` / `started_at` / `ended_at` はクライアント由来の値をそのまま保存する。軌跡からのサーバー再計算はしない。異常値は範囲・整合性のバリデーション（`ended_at > started_at`、24h 上限、未来日付の排除、時計ずれ 60 秒の許容）で弾く。
...
**ライフサイクルを「終了時1回」に絞ったのは、MVP に運用課題を持ち込まないため。** 開始時にレコードを作ると、必ず「終了しなかった散歩」が溜まり、その期限切れ判定・掃除・表示除外という仕事が increment ごとに増える。復帰要件は現時点では「アプリを落としても散歩が続く」で足り、それは mobile のローカル永続化で満たせる。将来サーバー側の進行中管理が必要になっても、nullable な `status` 列を後付けできる余地は残している。
...
- 進行中の散歩をサーバーから復元できない。端末をまたいだ散歩の継続もできない。
...
- SS-19 への申し送り: `client_walk_id` は散歩開始時に採番して `ActiveWalk` に持たせ、ローカル永続化する場合は一緒に保存する。`duration_seconds` には一時停止を除いた `elapsedSec` を送る。リクエストのフィールドは snake_case。

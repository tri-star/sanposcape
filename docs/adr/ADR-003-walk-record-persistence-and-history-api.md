...
走査は `(user_id, started_at DESC, id DESC)` インデックス順のチャンク読み（既定200行）+ ギャップ検出による早期打ち切りで行い、全件読みをしない。安全弁として `WALK_STATS_STREAK_MAX_DAYS = 3660`（10年）で打ち切る（異常データや将来のバルクインポートで走査が無限に伸びるのを防ぐため。現実的には到達しない）。この判定はチャンクを1回処理し終えるごと（既定200日区切り）にしか行わないため、打ち切りはちょうど3660日ではなく最大でチャンクサイズ未満のオーバーシュートを許容する概ねの上限である。チャンクの途中で厳密に打ち切ろうとすると走査ロジックの責務が増える割に実益が薄く（10年連続は実データでは起きない）、安全弁の目的はあくまで異常データによる無限走査の防止であって日数の厳密な打ち切りではないため、この設計にしている。
...
### 13. 散歩の削除は物理削除で行い、`DELETE /walks/{walk_id}` を提供する（SS-53 追補）

- 物理削除（論理削除カラム `deleted_at` は導入しない）。成功時は 204 No Content。
- 認可は決定6と同じ。`WalkRepository.delete()` も `user_id` を必須キーワード引数に取り、他ユーザーの散歩・存在しない ID・削除済み ID はいずれも 404。
...
- **選択肢1: 物理削除（採用）** — メリット: 既存クエリ（一覧・集計・streak）に除外条件を足す必要がなく、除外漏れによる不整合が構造的に起きない。アカウント削除（CASCADE の物理削除）と挙動が対称。デメリット: 復元不可。監査ログが残らない。
- **選択肢2: 論理削除（`deleted_at`）** — メリット: 誤削除から復元できる。将来のゴミ箱 UI に繋がる。デメリット: `get_by_id` / `list_for_user` / `aggregate_daily_for_user` / `list_walk_dates_desc` の全クエリに除外条件が必要で、1箇所の漏れが集計値を狂わせる（決定11で非正規化を避けたのと同種のリスク）。MVP に復元要件が無い。
...
**冪等化を最初から入れたのは、散歩終了時の保存が最も失敗しやすい瞬間だから。** 屋外・電波不良の状況で送信するため SS-19 は必ずリトライする。冪等でなければ同じ散歩が履歴に二重に並び、しかもユーザーには削除手段がない（削除 API は今回スコープ外）（**SS-53 追補**: 削除 API を追加したため削除手段はあるが、二重登録を防ぐ責務は引き続き冪等キーが持つ）。冪等キーを**保存直前でなく散歩開始時に採番する**のは、リトライのたびに値が変わると冪等性が効かないため。
...
**（SS-42 追補）streak を非正規化カラムにしなかったのは、更新漏れ・再計算コストを避けるため。** 保存すると散歩の追加・（将来の）削除（削除は SS-53 で実装済み）・編集のたびに再計算が必要になり、更新を1箇所でも忘れると値が腐る。`(user_id, started_at DESC, id DESC)` インデックスの順序が JST 暦日の降順と単調に一致することを利用し、チャンク読み + 早期打ち切りで実用的なコストに抑えられると判断した。
...
**（SS-53 追補）削除を物理削除にしたのは、除外条件の書き漏れリスクを新たに作らないため。** 論理削除は読み取り側の全クエリに条件を配る形になり、`walks` は既に一覧・詳細・日次集計・streak 走査と読み取り経路が4本ある。1本でも漏らすと「消したはずの散歩が集計に残る」という発見しにくい不整合になる。復元要件が無い現時点では、除外条件をゼロに保てる物理削除の方が安全と判断した。

**（SS-53 追補）削除 API を冪等にしなかったのは、冪等にするには tombstone が必要だから。** 「削除済み ID」と「存在しない ID」を区別して前者に 204 を返すには削除済み ID を記録し続ける必要があり、決定6（存在の有無を漏らさない）とも相反する。区別せず 404 を返し、クライアント側で「404 = 目的達成」と解釈する方が実装・セキュリティ両面で単純である。
...
- **（SS-53 追補）** 削除は集計 API に追加実装なしで反映される（決定10・11がクエリ算出で非正規化を持たないため）。keyset cursor は「行への参照」ではなく `(started_at, id)` の値比較なので、cursor が指していた散歩が削除されても次ページ取得は壊れない。
...
- **（SS-42 追補）** streak は安全弁としておよそ3660日（10年）で打ち切るため、それ以上の連続は頭打ちになる（決定11）。安全弁はチャンク（既定200日）を1回処理し終えるごとにしか判定しないため、打ち切りはちょうど3660日ではなく、最大でチャンクサイズ未満（既定で199日）のオーバーシュートを許容する。

...
- **写真バケットの S3 バージョニング**が有効かは未確認（infra 側の管理）。有効なら、削除した写真の
  実データがいつ消えるかは infra のライフサイクル設定次第になる（本文: SS-113 追補 決定28）
...
- **既定地図の削除を 409 で禁止する**: 規則は最も単純だが、地図が1つしか無いユーザーが
  その地図を消せなくなる。
...
マーカーを置くだけで、実データがいつ消えるかは infra のライフサイクル設定で決まる。
backend の実装はどちらでも変わらない（決定22 のピン削除も同じ）。

...
[ADR-008](./ADR-008-deploy-release-separation.md) として起こし、該当箇所に転記先を注記している
（決定そのものは移していない。本 ADR は backend の公開方式の記録として読めるまま）。
...
- **production へのデプロイが成功したら、デプロイした SHA に `backend/vX.Y.Z` タグと GitHub Release を作る。**
  ...
  - デプロイとリリース（利用者への機能の公開）の分離（AppConfig によるフィーチャーフラグ、ストアの手動リリース）は、
    別途 ADR を起こす予定。（**SS-104 追補**: [ADR-008](./ADR-008-deploy-release-separation.md) として起こした。
    本追補で決めたタグ・GitHub Release の扱いは ADR-008 の決定4・決定5 に引き継がれ、
    `<app>/vX.Y.Z` という形はアプリ横断の規則になった。運用手順は

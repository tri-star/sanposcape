...
2026-09-06（初版）、2026-09-06 追補（SS-70）、2026-09-07 追補（SS-78）、2026-09-11 追補（SS-78 その2）、2026-09-15 追補（SS-72）
...
Permission Boundary の導入とデプロイのトリガー設計を末尾の「SS-72 追補」に記録している。
...
- [x] SAM デプロイの CI 化（OIDC ロールの整備後）。
      **（SS-72 追補）`.github/workflows/backend-deploy.yml` を追加した。** 実行ロールへの
      ...
      prod は infra 側の `live/account` の apply 待ち。
...
- **境界を初めて入れるデプロイだけは手元の管理者権限で行う。** 既存ロールへの境界の後付けは
  ...
  CI 側に例外を作らず、既存スタックのある dev で 1 回だけ手元から流す運用にした。
  prod はスタック未作成のため最初から境界付きで作られ、この手順は不要。
- **トリガー**: dev は main への push（デプロイ成果物に入るパスだけ）と手動実行、
  prod は手動実行のみ（main 限定 + Required reviewers）。backend CI を通過したコミットだけをデプロイする。
  `pull_request` 系のトリガーは付けない。
  - 検討した代替: dev も手動実行のみにする案は、main とデプロイ内容の乖離が起きやすく
    「main に載ったものが dev で動いている」前提を崩すため採らなかった。

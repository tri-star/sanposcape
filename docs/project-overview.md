...
| 往復範囲 | 現在地から指定時間内に徒歩で往復できる地理的範囲。スポット候補の絞り込み（5.2）では片道時間・距離の「×2×補正後の目安」で判定する（実測係数 LOOP_FACTOR で補正。詳細は [ADR-001](./adr/ADR-001-map-poi-google-maps-platform.md) の SS-33 追補を参照） |
| 散歩ルート (Walking Route) | スポット選択時に提示される徒歩の推奨経路。既定では**周回（現在地→目的地→別経路→現在地）**で提示し、往路/復路をそれぞれ「leg」として分けて持つ（SS-33。周回にできない場合は復路が往路と同じ道になるフォールバックがある）。実際に歩いた軌跡は「軌跡 (Track)」として別語に分ける（SS-18 でコード上も分離）。**Expo Router / React Router の「ルート(route＝画面/URL)」とは別概念**。コード上は walking-route / walkingRoute 等と表記し、画面遷移の route と混同しない |

...
> **未実装**。SS-98（backend の読み取り基盤と `/app-config`）と
> SS-99（切り替えワークフロー）の完了後に、確定した手順でここを書き直すこと。
...
> 判断材料となる最低サポートバージョンの仕組みは**未実装**（SS-98 / SS-101）。

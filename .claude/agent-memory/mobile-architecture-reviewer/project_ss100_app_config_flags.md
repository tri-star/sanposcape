...
追加される差分を見たら、`src/api/endpoints/` のようなサブフォルダへの分割ルールが
folder-structure.md に足されたか、あるいは依然としてフラットに積まれ続けているかを確認する。
...
**How to apply**: 次に実際に画面単位のフラグガード（`app/`ルートで`resolveFeatureGateDecision`
→ `<Redirect>`）が実装されたら、architecture-guideline.md に具体例が追記されたか確認する。
追記されないまま「レシピを参照」という文言だけが増えているなら指摘する。

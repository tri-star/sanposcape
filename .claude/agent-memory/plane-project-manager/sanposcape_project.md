...
## 2026-09-17追加: SS-91「mobile: 散歩サマリ画面の数値重なり表示バグ」を新規作成

- SS-91（work_item_id: `2009e1d5-578b-484e-b6bd-867e5d617973`）を新規作成、State=Backlog、label=`bugfix`（`b14a634a-8edf-494a-8007-15b822c60ef7`）、priority/module/担当者は未設定。SS-33（周回ルート、PR #83）のエミュレータ確認中に発見した既存バグ（散歩サマリ画面で歩行距離・歩数の桁が大きいと経過時間/距離/歩数が横に重なる）。
- ラベルは既存の`bugfix`（「既存動作の不具合修正」、色`#EF4444`）を使用。プロジェクトのラベル一覧（2026-09-17時点、6件）: bugfix, infra, manual-required, small, docs, ready。
...
## 2026-09-17追加: SS-33をReview化・PR #83リンク登録（PR #62クローズ後の再チャレンジ）

- SS-33（work_item_id: `62f9d861-9139-4b9c-ba34-7eba803b61c2`、モジュールA「MVP達成に必要な残課題」所属）をIn Progress→Review（`65b14f74-6fd7-4129-9fab-60908f844572`）に更新。
- 既存の`create_work_item_link`でPR #62（`https://github.com/tri-star/sanposcape/pull/62`、link_id `95812738-1142-4256-aa50-95836374618f`、2026-08-23登録）はクローズ済みだが削除せず残したまま、PR #83（`https://github.com/tri-star/sanposcape/pull/83`、link_id `14f53ec5-5109-4eaf-93d6-d23bfc873915`）を新規追加登録。あわせてコメント（comment_id: `909a8367-a6fd-4005-abe4-183b31b5d0ab`）で「PR #62クローズ後の再チャレンジ。散歩中の再計算を撤去し、周回ルートを参考表示にする方針で実装」を記録。
- **Why**: ユーザー指示。SS-15のケース（2026-07-30メモ）と同様、旧PRがクローズ済みでも既存リンクは削除せず新PRを追加登録し、コメントで経緯を補足する運用を継続。
- `list_work_item_properties`は確認していないが、これまでの全確認で常に空配列だったため今回は省略し、既知の方針（標準Link機能を使う）で直接登録した。
...
## 2026-09-17追加: SS-92「backend: 周回ルートの生成品質を改善する(同じ道フォールバック率を下げる)」を新規作成・SS-33とrelates to設定

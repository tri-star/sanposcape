...
**設計の骨子（正本は ADR-003 SS-42 追補 決定10〜12。プラン文書は gitignore 対象で既に消失、[[antipattern_plan_decision_refs]]）:**
...
- `stats.py:23` / `service.py:148` の `（4.4 節参照）` コメントは既に消失したプラン文書を指しており [[antipattern_plan_decision_refs]] と同型（追跡不能）。ただし同じPRで `ADR-003` 決定11に同内容が git 管理下で書かれているため、実害は小さいLow指摘に留めた。このPRの他のコメントは概ね `ADR-003 SS-42 追補 決定10` のようにコミット済みドキュメントを正しく参照しており、良い実践として広がりつつある。

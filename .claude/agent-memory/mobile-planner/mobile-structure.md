...
**services 層の seam パターン**: `src/services/<svc>/{types.ts,index.ts,<svc>.stub.ts,<svc>.real.ts}`。index が `process.env.EXPO_PUBLIC_*` で real/stub 選択。呼び出し側は index/types のみ参照。単体テストは常に stub、E2E(Maestro)は再現可能なら real。SS-8 時点では services/ は空（.gitkeep のみ）で auth/location 未実装。

**API**: Orval 生成物は `src/api/generated/`（手編集禁止、`pnpm --filter mobile orval` で再生成）。現状 `health` と `spots` のみ生成済み。**walks/履歴・散歩記録保存の API は未定義**（履歴/保存を実データ化するなら backend 伝達が必要）。クライアントは `src/api/client.ts`（customFetch）+ `queryClient.ts`。msw は使わない方針。

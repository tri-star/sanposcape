...
SS-111（`GET /pins`一覧・`GET /pins/{pin_id}`詳細・`GET /pins/{pin_id}/photos`全件）はレビュー済み・Critical/High指摘なし。バックエンド実装プラン（D1〜D11、bbox必須パラメータ化・keysetページング設計・原本URLの扱い等）が実装に一貫して反映されている（プランと実装の乖離なし）。設計判断の一次記録は ADR-009 追補（決定14〜18）。
...
- **ストレージ未構成・障害時は503にせずURLをnullで返す**（D11）。閲覧系APIはDB由来の情報だけで200を返せる設計を維持する。
...
**How to apply:** SS-112以降のレビューでは、このメモの確立パターンからの逸脱を優先的に確認する。「なぜtotal_countや権限フィールドが無いのか」はSS-111のD9/D10で意図的にスコープ外とされた決定であり、再指摘しない（ADR-009追補決定14〜18に正式記録済み）。

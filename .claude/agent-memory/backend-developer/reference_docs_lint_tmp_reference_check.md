...
- どうしても `tmp/` の運用ルール自体を説明する必要がある行には `tmp-ref-ok` というコメントを
  添えると例外扱いになる。
- レビューで「コメントが `tmp/` のプラン文書や、その決定番号（`D4` など）を参照している」と
  指摘されて一掃するときは、まず `git diff <base>...HEAD --stat` で自分のブランチの差分ファイルに
  絞ってから grep する。既存コードに残る別チケットの参照まで巻き込むと差分が膨らむ（SS-111 の
  R1 対応で確認）。置換先は ADR の決定番号にする。

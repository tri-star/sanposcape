...
（新規201/再送200）・keyset(cursor)ページネーション。設計の詳細決定（D1〜D11）の正本は
ADR-003（`docs/adr/ADR-003-walk-record-persistence-and-history-api.md`）。
...
- 次のフロントエンド側タスクは SS-19/SS-20（mobile 側の記録保存・履歴表示）。backend の
  ...
  メモリと ADR-003、`packages/backend/openapi.yaml` を参照すれば分かる。

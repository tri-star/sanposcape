...
import { getRoundTripRoute } from "@/api/generated/endpoints/explore/explore";
import type { RoundTripRouteRequest } from "@/api/generated/model";
import { toRoundTripWalkRoute } from "@/features/walk/lib/walkRoute";
import type { WalkRoute } from "@/features/walk/types";

/**
 * /explore/routes/walking/round-trip を叩いて WalkRoute を返す fetcher。
...
export async function fetchWalkRoute(
  ...
  const response = await getRoundTripRoute(request, { signal: options?.signal });
  if (response.status !== 200) {
    // customFetch は非2xx で ApiError を throw するため通常ここには来ない（型の網羅のため）。
    throw new ApiError(response.status);
  }
  return toRoundTripWalkRoute(response.data, options?.destinationName);
}

...
import { MIN_REGION_DELTA, type MapRegion } from "@/lib/mapRegion";
 ...
 * 高緯度で発散しないよう cos は 0.1 で下限クランプし、delta には最小値 `MIN_REGION_DELTA` を設ける。
 */
export function regionForRoundTrip(center: GeoCoordinates, durationMin: number): MapRegion {
  ...
  const latitudeDelta = Math.max(MIN_REGION_DELTA, diameterMeters / METERS_PER_DEGREE_LATITUDE);

  const cosLatitude = Math.max(MIN_COS_LATITUDE, Math.cos((center.latitude * Math.PI) / 180));
  const longitudeDelta = Math.max(MIN_REGION_DELTA, latitudeDelta / cosLatitude);

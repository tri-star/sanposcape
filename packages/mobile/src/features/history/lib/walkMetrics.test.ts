...
describe("formatPace", () => {
  ...
  it("distanceMeters は表示用に丸めた km ではなく生の値で計算する（丸め誤差の回帰検知）", () => {
    // 2143m/1920秒: 表示用に丸めた2.1kmで計算すると15'14"/km（約2%ズレ）になってしまうが、
    // 生の2143mで計算すると14'56"/kmになるはず。
    expect(formatPace(1920, 2143)).toBe("14'56\"/km");
  });
});

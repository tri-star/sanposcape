...
- **(b)** ナビタブで散歩をしていないときに FAB を出す。FAB から全画面の地図を現在地起点で開き、
  長押しした地点を位置として登録画面へ進む。散歩中は、散歩中画面の地図の長押しで同じように
  登録画面へ進む（PR #102 の動作確認後の追加要望）。
...
- **D-ユーザー決定（PR #102 の動作確認後の追加要望）: FAB はアイコンだけの円形にする**
  （`IconButton` の `filled` / `lg`。文言付きのボタンは横に大きすぎたため）。スクリーンリーダー向けの
  ラベルは「地図からピンを置く」のまま。
- **D-ユーザー決定（同上）: 散歩中も、散歩中画面の地図を長押しした地点でピン登録へ進める。**
  長押しした座標と `clientWalkId` を付けて `/pins/new` へ **push** する（戻ると散歩中画面に戻る。
  「この場所にピンを追加」と同じ）。長押しは地図を触っているうちに意図せず起きうるため、
  フラグ OFF・座標不正のときはトーストも出さずに何もしない（`features/walk/lib/addPinAction.ts` の
  `resolveMapLongPressPinAction`。`features/walk` は `features/pin` を import しない規約のまま）。
...
- **D5: 現在地が取れない（権限拒否・取得失敗）とき、地点選択画面は日本全体を初期表示にする。**
  範囲は日本の端（南端 波照間島・西端 与那国島・北端 宗谷岬・東端 納沙布岬。遠い離島は除く）に
  余白を足して計算する（`pinLocationPicker.ts` の `JAPAN_DISPLAY_BOUNDS`。PR #102 のレビューで、
  固定値では稚内・那覇・石垣が外れていたため）。
...
- **(a)(b) の地図操作は支援技術（スクリーンリーダー等）で完全には代替できない**。
  `PinMapFullScreen` は地図を包む `View` を `accessible + accessibilityLabel` で1ノードに畳んで
  おり（`MapView`/`Marker` は個別に読み上げ対象にしない。`pages-components-guideline.md`
  「地図オーバーレイは a11y 対象外」の既定方針と同じ）、タップ/長押しで座標を選ぶという操作の
  核心はジェスチャー前提で、緯度経度の直接入力・住所検索・カードからの選択のような代替入力手段は
  今回作っていない。`accessibilityHint` で「地図をタップ/長押しすると位置を選べる」ことは伝えて
  いるが、スクリーンリーダー利用者がその操作自体を実行できるようにはならない。恒久対応は別途
  フォローアップで検討する（レビュー指摘）。
- **Android では選択済みピン（`Marker`）の真上をタップしても位置が動かない場合がある**。
  `react-native-maps` の `Marker` は Android で `onPress` を `MapView` 側へ伝播しない仕様
  （`stopPropagation`/`tappable` のどちらでも回避できない。react-native-maps #1132）があり、
  (a) 位置調整で「今あるピンのすぐ近くをタップして微調整する」という最も起こりやすい操作が、
  Marker の当たり判定に重なった場合だけ反応しないことがある（レビュー指摘）。回避策としては
  ピンの近くを少しずらしてタップし直すか、`Marker` を一旦動かしてから狙った位置を再タップする
  程度で、根本的な回避策は無い（`Marker` に `onLongPress` は無く、長押しで代替できるかは未検証）。
  地図をタップし直しても反応しないように見える場合の案内は今回追加しない（フォローアップ候補）。

...
- **地図の描画と外部データは E2E の assert 対象にしない**（SS-15 で確立）。CI の preview APK には
  ...
  **（SS-21 追補）**
  ...
  - 一方で **`/explore/places` が常に 503 という前提は、backend に `MAPS_MODE=fake`（決定的な
    fake provider）を入れることで解消**し、MVP 主要フロー（探索 → 散歩開始 → 終了 → 保存 →
    履歴）を E2E の対象にする（backend 対応は SS-44 として別タスクに切り出し済み）。
  - **候補の件数・名称・距離/時間の値は引き続き assert しない**。assert するのは「候補が 1 件
    以上ある（`spot-card-0` が存在する）」までとする。

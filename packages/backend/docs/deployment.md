...
| `GET /pins` 等の閲覧系は 200 だが `thumbnail`/`original_url` が常に null | 環境変数が空（`UnconfiguredObjectStorage`）、または実行ロールに `original/*`・`thumb/*` への `s3:GetObject` が無い。閲覧系は 503 にしない設計（ADR-009 決定18）なので、書き込み系のように 5xx では気付けない | 上の確認 2) で環境変数を確認する。空でなければ確認 3) でポリシーの `Resource` を確認する |
